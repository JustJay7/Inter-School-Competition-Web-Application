...
namespace DataAccess
{
    public class AdminClass : UserClass
    {
        ...
        public override int LoginCheck()
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM AdminLoginCredentials WHERE AdminEmail='" + this.UserEmail + "' and AdminPassword='" + this.NewPassword + "'", con);
            cmd.Parameters.AddWithValue("@AdminEmail", this.UserEmail);
            cmd.Parameters.AddWithValue("@AdminPassword", this.NewPassword);
