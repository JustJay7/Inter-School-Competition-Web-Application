...
namespace DataAccess
{
    public class EventCoordinatorClass : UserClass
    {
        ...
        public override int LoginCheck()
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["mycon"].ToString());
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM EventCoordinatorCredentials WHERE EventCoordinatorEmail='" + this.UserEmail + "' and EventCoordinatorPassword='" + this.CurrentPassword + "'", con);
            cmd.Parameters.AddWithValue("@EventCoordinatorEmail", this.UserEmail);
            cmd.Parameters.AddWithValue("@EventCoordinatorPassword", this.CurrentPassword);
